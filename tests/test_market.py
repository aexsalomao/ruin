...
class TestBeta:
    ...
    def test_correlated_beta(self, daily_returns: pl.Series, benchmark_returns: pl.Series) -> None:
...
class TestAlpha:
    ...
    def test_alpha_type(self, daily_returns: pl.Series, benchmark_returns: pl.Series) -> None:
...
class TestCorrelation:
    def test_self_correlation_is_one(self, daily_returns: pl.Series) -> None:
        assert math.isclose(correlation(daily_returns, daily_returns), 1.0, rel_tol=1e-6)

    def test_anticorrelation_is_minus_one(self, daily_returns: pl.Series) -> None:
        assert math.isclose(correlation(daily_returns, -daily_returns), -1.0, rel_tol=1e-6)

    def test_range(self, daily_returns: pl.Series, benchmark_returns: pl.Series) -> None:
...
class TestCapture:
    ...
    def test_down_capture_self_is_one(self, daily_returns: pl.Series) -> None:
        assert math.isclose(down_capture(daily_returns, daily_returns), 1.0, rel_tol=1e-6)
