...
Repo-specific guidance. Global defaults: `C:\Users\axsal\dev\CLAUDE.md`. Cross-project rules: `.claude/rules/code-style.md`, `.claude/rules/testing.md`. Only rules unique to ruin belong here.
...
```
src/ruin/
  _internal/normal.py      # norm_ppf, norm_pdf — standard-normal helpers with no SciPy dep
...
- **Internal math stays in Float64** to avoid catastrophic cancellation in `cum_prod`, variance accumulators, autocorrelation adjustments. `to_series` / `to_dataframe` cast inputs to `INTERNAL_FLOAT_DTYPE` on entry.
...
- In tests, use `FLOAT32_REL_TOL = 1e-5` / `FLOAT32_ABS_TOL = 1e-6` from `tests/conftest.py` for Float32 outputs. Float64 scalars can use `1e-9` — `1e-9` on a Float32 value is noise.

## Ruin-specific typing

- `ReturnInput` alias = `pl.Series | pl.Expr | np.ndarray | pl.DataFrame`. All public functions accept it.
- Every module starts with `from __future__ import annotations`.
- Import `Callable` from `collections.abc`, not `typing`.
- Frozen dataclasses (`JarqueBeraResult`) for structured returns.

## Input handling

- All public functions route through `to_series` (or `to_dataframe` for multi-stream paths). Never branch on input type inside metric code.
